...
df_final_train['raw_target'] = df_final_train.target
min_target = df_final_train.raw_target.min()
df_final_train['target'] = df_final_train.raw_target.map(lambda x: math.log((x-min_target)+1))
sns.distplot(df_final_train.target)
...
df_output['raw_target'] = preds
df_output['target'] = df_output.raw_target.map(lambda x: (math.exp(x) + min_target) - 1)
df_output[['card_id','target']].to_csv("C:/Users/Garrett/Documents/Kaggle/Elo/submissions/submission_8.csv", index=False)


def grid_search(df, features):
    num_trees = [100, 200]
    min_samples = [0.0001,0.001]
    for t in num_trees:
        for s in min_samples:
            grid_rfr = RandomForestRegressor(n_estimators=t, min_samples_split=s)
            grid_rfr.fit(df[features], df.target)
            r = grid_rfr.score(df[features], df['target'])
            print 'Num Trees: ' + str(t) + '\tMin Samples: ' + str(s)
            print 'R^2: ' + str(r)